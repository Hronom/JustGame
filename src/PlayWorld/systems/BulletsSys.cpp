...
void BulletsSys::injectUpdate(const float &xTimeSinceLastUpdate)
{
    ...
    xEntitys = JGC::WorldsSystem::instance()->getActiveWorld()->getEntitysInNode(Nodes::Bullets);

    for(int i = 0; i < xEntitys.size(); ++i)
    {
        BulletCom *xBulletCom;
        xBulletCom = static_cast<BulletCom*>(xEntitys.at(i)->getComponent(ComponentsTypes::BulletCom));
